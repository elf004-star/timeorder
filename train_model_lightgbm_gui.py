...
class TrainingGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("井段状态预测模型训练系统LightGBM")
                ...
                for dataset, metrics in best_score.items():
                    if hasattr(metrics, 'items'):
                        for metric_name, score in metrics.items():
                            self.log(f"最佳分数 ({dataset} - {metric_name}): {float(score):.6f}")
            else:
                self.log(f"最佳分数: {best_score}")
