...
class WellStatusPredictor:
    ...
    def load_model(self, model_path: str):
        ...
        # 加载模型 (pickle format for sklearn)
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        self.model = GradientBoostingClassifier(
            n_estimators=1,  # Start at 1 to satisfy validation
            ...
            warm_start=True,
            random_state=42,
            n_iter_no_change=None # We handle early stopping manually
        )
        ...
        for i in range(1, n_estimators + 1):
            ...
            # 记录Loss
            # train_score_ contains deviance (loss) for each stage
            train_loss = self.model.train_score_[-1]
            ...
            if callback and i % 5 == 0: # Update less frequently to avoid UI freeze
        ...
        # Revert to best iteration (optional but recommended)
        # We simulate "reverting" by slicing the internal estimators
        if best_iteration < len(self.model.estimators_):
            self.model.estimators_ = self.model.estimators_[:best_iteration]
            self.model.train_score_ = self.model.train_score_[:best_iteration]
            self.model.n_estimators = best_iteration
            print(f"模型回退到最佳迭代: {best_iteration}")
    ...
    def save_model(self, filename: str = "well_status_model_gbm.pkl", params: Dict = None):
        """保存模型到Models文件夹"""
        ...
        if params is not None:
            params_info = {
                ...
                'model_file': filename,
                'best_iteration': getattr(self.model, 'n_estimators', 0),
                'parameters': params
            }
            
            params_filename = filename.replace('.pkl', '_params.json')
...
class TrainingGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("井段状态预测模型训练系统 (GBM)")
    ...
    def setup_ui(self):
        ...
        # 超参数设置
        ttk.Label(control_frame, text="GBM模型超参数", font=('Arial', 10, 'bold')).grid(row=row, column=0, columnspan=3, sticky=tk.W, pady=5)
        row += 1
        ...
        # Max Leaf Nodes (similar to num_leaves)
        ttk.Label(control_frame, text="最大叶子节点:").grid(row=row, column=0, sticky=tk.W, pady=5)
        ...
        self.num_boost_var = tk.IntVar(value=200)
        ...
        self.early_stop_var = tk.IntVar(value=20)
        ttk.Entry(control_frame, textvariable=self.early_stop_var, width=15).grid(row=row, column=1, sticky=tk.W, pady=5, padx=5)
        row += 1
        
        # Min Samples Leaf
        ttk.Label(control_frame, text="最少叶子样本:").grid(row=row, column=0, sticky=tk.W, pady=5)
        ...
        self.bagging_frac_var = tk.DoubleVar(value=1.0)
    ...
    def train_model(self):
        ...
        try:
            ...
            # 4. 设置参数
            params = {
                'n_estimators': self.num_boost_var.get(),
                'learning_rate': self.lr_var.get(),
                'max_leaf_nodes': self.num_leaves_var.get(),
                'max_depth': self.max_depth_var.get(),
                'min_samples_leaf': self.min_child_var.get(),
                'max_features': self.feature_frac_var.get(),
            ...
            memory_dir = os.path.join(base_dir, "memory_gbm")
    ...
    def run_prediction(self, model_path: str, data_path: str, output_path: str):
        ...
        try:
            ...
            if missing_cols:
                self.log(f"警告: 缺少以下特征列，将用0填充: {missing_cols}")
                for col in missing_cols:
                    df_with_features[col] = 0
            
            X_pred = df_with_features[feature_cols].values
            X_pred = self.predictor.scaler.transform(X_pred)
            self.progress_var.set(70)
            
            # 5. 预测
            self.log("执行预测...")
            predictions_proba = self.predictor.predict_proba(X_pred)
            predictions = np.argmax(predictions_proba, axis=1)
            
            # 6. 规则修正
            corrected_predictions = self.predictor.apply_rules(df_with_features, predictions, predictions_proba)
            self.progress_var.set(90)
            
            # 7. 保存结果
            self.log(f"保存结果: {output_path}")
            
            result_df = df.copy()
            result_df['origin_status_predict'] = predictions
            result_df['status_predict'] = corrected_predictions
            
            # 添加状态描述
            status_map = {0: 'Vertical', 1: 'Build-up', 2: 'Hold', 3: 'Drop-off'}
            result_df['status_desc'] = result_df['status_predict'].map(status_map)
